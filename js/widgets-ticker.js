...
(function () {
    ...
    function updateMobilePortalData() {
        ...
        // تحديث البيانات من APIs
        Object.entries(apiData.currency).forEach(([key, value]) => {
            const el = document.getElementById('mobile-portal-' + key);
            if (el) el.textContent = value;
        });
        Object.entries(apiData.gold).forEach(([key, value]) => {
            const el = document.getElementById('mobile-portal-' + key);
            if (el) el.textContent = value;
        });
        ...
        Object.entries(apiData.market).forEach(([index, data]) => {
            const el = document.getElementById('mobile-portal-market-' + index);
            if (el) el.textContent = data.change || data.value;
        });
    }
    ...
    function updatePortalData() {
        ...
        // تحديث البيانات من APIs
        Object.entries(apiData.currency).forEach(([key, value]) => {
            const el = document.getElementById('portal-' + key);
            if (el) el.textContent = value;
        });
        Object.entries(apiData.gold).forEach(([key, value]) => {
            const el = document.getElementById('portal-' + key);
        ...
        Object.entries(apiData.market).forEach(([index, data]) => {
            const el = document.getElementById('portal-market-' + index);
            if (el) el.textContent = data.change || data.value;
        });
    }
