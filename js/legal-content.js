 ...
 * js/legal-content.js
 * ------------------------------------------------------------------
 * 🆕 المصدر الموحّد الوحيد لثلاثة محتويات عبر كامل المنصة:
 *      1) دليل الاستخدام (Guide)
 *      2) شروط الاستخدام وإخلاء المسؤولية (Terms)
 *      3) سياسة الخصوصية وحماية البيانات (Privacy)
 *
 * الفكرة: بدل تكرار نفس النصوص داخل كل صفحة (index.html، no-map-search.html،
 * شاشة الترحيب الترويجية، شاشة التسجيل...) بمودالات منفصلة قد تختلف عن بعضها
 * مع الوقت عند أي تعديل مستقبلي، أصبح المحتوى الفعلي معرَّفاً هنا مرة واحدة
 * فقط. أي تعديل بهذا الملف ينعكس تلقائياً وفوراً في كل صفحة تُحمِّله.
 *
 * طريقة الاستخدام بأي صفحة:
 *   1) حمّل هذا الملف (سكربت عادي، قبل إغلاق body يكفي).
 *   2) ضع أي عنصر تريد أن يفتح أحد المحتويات وأعطه الخاصية:
 *         data-legal="guide"    → يفتح دليل الاستخدام
 *         data-legal="terms"    → يفتح شروط الاستخدام
 *         data-legal="privacy"  → يفتح سياسة الخصوصية
 *      (يعمل مع <button> أو <a> أو أي عنصر قابل للنقر)
 *   3) العناصر بالمعرّفات (IDs) الشائعة التالية تُربط تلقائياً بدون الحاجة
 *      لإضافة data-legal يدوياً (للتوافق مع الأزرار الموجودة مسبقاً بالصفحات):
 *         #btn-user-guide, #footer-guide-search, #footer-guide-provider
 *             → دليل الاستخدام
 *         #footer-terms-btn, #promo-terms-link, #welcome-terms-link
 *             → شروط الاستخدام
 *         #footer-privacy-btn, #promo-privacy-link, #welcome-privacy-link
 *             → سياسة الخصوصية
 *
 * المودال نفسه يُبنى ديناميكياً مرة واحدة فقط ويُعاد استخدامه، فلا حاجة
 * لأي HTML إضافي بالصفحة نفسها.
 * ------------------------------------------------------------------
 */
(function () {
    'use strict';
