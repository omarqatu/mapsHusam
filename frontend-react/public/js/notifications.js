...
class NotificationSystem {
    ...
    connect() {
        ...
        // 🆕 [نظام طلب الخدمة والدردشة]: نكتفي هنا بتحويل أحداث السوكيت إلى
        // أحداث DOM عامة (CustomEvent) بدل التعامل معها مباشرة، حتى يستطيع
        // ملف service-chat.js (المسؤول عن واجهة الطلب/الدردشة) الاستماع لها
        // دون الحاجة لأي مرجع مباشر لكائن السوكيت هنا.
        this.socket.on('service_request_new', (data) => {
            document.dispatchEvent(new CustomEvent('serviceRequestNew', { detail: data }));
        });
        this.socket.on('service_request_response', (data) => {
            document.dispatchEvent(new CustomEvent('serviceRequestResponse', { detail: data }));
        });
        this.socket.on('service_request_message', (data) => {
            document.dispatchEvent(new CustomEvent('serviceRequestMessage', { detail: data }));
        });
        this.socket.on('service_request_completed', (data) => {
            document.dispatchEvent(new CustomEvent('serviceRequestCompleted', { detail: data }));
        });
